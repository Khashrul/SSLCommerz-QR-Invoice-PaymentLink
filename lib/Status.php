...
class Status
{
    ...
    public function __construct($storeid, $storepass, $post_data, $sandbox = false)
    {
        ...
        $this->setEnv($this->sandbox, $this->config['apiUrl']['payment_status']);
        $this->getStatusResponse($this->config['connect_from_localhost']);
        $this->getResponse();
    }

    protected function setResponse($statusResponse = [])
    {
    ...
    public function setParamInfo(array $info)
    {
        ...
        $this->data['action'] = "invoicePaymentStatus";

        return $this->data;
    }

    public function getStatusResponse($setLocalhost = false)
    {
        $curl = curl_init();
